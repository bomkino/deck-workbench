...
Each PSD has **00.Background** and **01.Character**, sharing one embedded **Shared Artwork.psd**. Edit that shared artwork in Photoshop to change both instances. Its chosen images retain their frames, fit, crop transforms and masks, with native guides and a nonprinting numbered-grid path. Both outer roles initially show the complete artwork: the artist must create the character cut-out/mask. Then use the separate PNG role exporter for the artwork-only background/character PNGs. No automatic cut-out is produced. Text and gradients remain design work in the destination; use the prototype PDF as the visual guide and the writing importer for copy. The existing InDesign production builder can consume the prepared bundle; Figma writing import and artwork placement remain separate steps.

PSD generation does not require Photoshop, but opening/editing in Photoshop and placing the bundle in InDesign are separate verification steps. The implementation does not establish native Adobe acceptance or a finished deck. PSD export is serial, supports at most 12 placed images per slide, and enforces per-slide source and memory budgets; see [Known boundaries](KNOWN_LIMITATIONS.md).
