...
test('large Interface Scale reflows the phased workspace instead of clipping it', () => {
  ...
  assert.match(styles, /data-workspace-layout="two-column"\] \.plan-phase \{[\s\S]+?grid-template-areas: "tools tools" "sequence map" "editor editor"/)
  assert.match(styles, /data-workspace-layout="single-column"\] \.plan-phase \{[\s\S]+?grid-template-areas: "tools" "sequence" "map" "editor"/)
