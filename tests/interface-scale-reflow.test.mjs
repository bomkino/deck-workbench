...
test('large Interface Scale reflows the phased workspace instead of clipping it', () => {
  ...
  assert.match(styles, /data-workspace-layout="single-column"\] \.curate-tray \{[\s\S]+?grid-template-rows: repeat\(5, minmax\(7rem, auto\)\)/)
  assert.match(styles, /data-workspace-layout="single-column"\] \.toolbar-actions \{[^}]*overflow: visible;/)
  assert.match(styles, /\.appearance-panel \{ position: absolute;/)
  assert.match(styles, /data-navigator-collapsed="true"\] \.sequence/)
  assert.match(styles, /data-inspector-collapsed="true"\] \.assembly-controls/)
  assert.match(styles, /data-workspace-layout="two-column"\][^\n]*data-navigator-collapsed="true"\][^\n]*\.plan-phase,[\s\S]*grid-template-rows: minmax\(0, 1fr\)/)
  assert.match(styles, /data-workspace-layout="two-column"\][^\n]*data-navigator-collapsed="true"\][^\n]*\.curate-phase,[\s\S]*grid-template-rows: minmax\(0, 1fr\) minmax\(8\.5rem, 18vh\)/)
  assert.match(workspace, /dataset\.navigatorCollapsed = String\(navigatorCollapsed\)/)
  assert.match(workspace, /toggleNavigator\.setAttribute\('aria-pressed', String\(!navigatorCollapsed\)\)/)
  assert.match(workspace, /if \(!projection\) \{\s*navigatorCollapsed = false\s*inspectorCollapsed = false/)
  assert.match(styles, /data-workspace-layout="single-column"\] \.stage-scroll \{[\s\S]+?overflow: auto; place-items: start/)
...
test('native shell leaves document actions to menus and never feeds scale into artboard geometry', () => {
  ...
  assert.doesNotMatch(app, /toolbarHeight|AdaptiveToolbarLabelStyle|PhosphorGlyph/)
