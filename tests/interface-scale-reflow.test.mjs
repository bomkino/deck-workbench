...
test('large Interface Scale reflows the phased workspace instead of clipping it', () => {
  ...
  assert.match(styles, /--control-size: max\(2\.75rem, 44px\)/)
  ...
  assert.match(styles, /data-workspace-layout="single-column"\] \.curate-phase \{[\s\S]+?grid-template-rows: 20rem minmax\(46rem, auto\)[\s\S]+?minmax\(32rem, auto\)/)
