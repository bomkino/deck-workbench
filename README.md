...
`v0.0.5` is the current source and release line. Plan now defaults new work to Full Bleed and drives the initial Assembly. Assembly renders assigned images without distortion, offers Fit/Fill, Small/Medium/Large text, direct movement/resizing, role-specific shortlist swaps, and visual gradient controls. Its Slide rail keeps every Slide visible and canvas changes apply. Curate exposes obvious Primary assignment, thumbnail trays, a local fullscreen preview with ratings, and role-aware assignment to any Slide. On macOS, one full-content window integrates the native traffic lights with Workbench’s compact phase toolbar while File and View commands remain in the system menu bar. The public GitHub Release, not version text alone, is the authority for whether those exact-commit artifacts are released and Latest.
...
Use the CI-built assets on the [`v0.0.5` release](https://github.com/bomkino/deck-workbench/releases/tag/v0.0.5) only when GitHub shows that release as public and Latest. Verify the adjacent `.sha256` file before opening an asset.
