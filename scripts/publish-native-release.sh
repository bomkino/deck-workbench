...
import json, sys
rows = [a for a in json.load(open(sys.argv[1]))['artifacts'] if not a['expired'] and a['name']==sys.argv[2] and a.get('workflow_run',{}).get('head_branch') in ['main','codex/slide-editing']]
...
gh release create "$TAG" "$ZIP" "$ZIP.sha256" "$RECEIPT"   --target "$SHA" --title "Deck Workbench $TAG — Native Mac slide editing"   --notes-file docs/RELEASE_NOTES.md --latest
