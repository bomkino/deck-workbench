...
assert.equal(checkpoint.revision, 34)
assert.equal(records.length, 34)
assert.deepEqual(records.map((record) => record.revision), Array.from({ length: 34 }, (_, index) => index + 1))
assert.deepEqual(records.map((record) => record.operation), [
  ...Array(9).fill('command'),
  'undo', 'redo',
  ...Array(7).fill('command'),
  ...Array(8).fill('undo'),
  ...Array(8).fill('redo'),
])
...
assert.equal(createResult.revision, 18)
assert.equal(createResult.journalReplayRevision, 18)
...
assert.equal(createResult.crashRecoveryRevision, 18)
...
assert.equal(reopenResult.reopenedRevision, 18)
assert.equal(reopenResult.undoSectionRevision, 19)
assert.equal(reopenResult.undoSlideRevision, 20)
assert.equal(reopenResult.undoContentRevision, 21)
assert.equal(reopenResult.undoSectionMoveDownRevision, 22)
assert.equal(reopenResult.undoSectionMoveUpRevision, 23)
assert.equal(reopenResult.undoSequenceMoveDownRevision, 24)
assert.equal(reopenResult.undoSequenceMoveUpRevision, 25)
assert.equal(reopenResult.undoParagraphUpdateRevision, 26)
assert.equal(reopenResult.redoParagraphUpdateRevision, 27)
assert.equal(reopenResult.redoSequenceMoveUpRevision, 28)
assert.equal(reopenResult.redoSequenceMoveDownRevision, 29)
assert.equal(reopenResult.redoSectionMoveUpRevision, 30)
assert.equal(reopenResult.redoSectionMoveDownRevision, 31)
assert.equal(reopenResult.redoContentRevision, 32)
assert.equal(reopenResult.redoSlideRevision, 33)
assert.equal(reopenResult.redoSectionRevision, 34)
...
const sectionMoveDown = checkpoint.undoStack.at(-4)
...
const sectionMoveUp = checkpoint.undoStack.at(-5)
...
const sequenceMoveDown = checkpoint.undoStack.at(-6)
...
const sequenceMoveUp = checkpoint.undoStack.at(-7)
...
const paragraphUpdate = checkpoint.undoStack.at(-8)
