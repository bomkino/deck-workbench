...
assert.match(contents.get('apps/macos/Info.plist'), /<key>CFBundleShortVersionString<\/key>\s*<string>0\.0\.5<\/string>/)
assert.match(contents.get('apps/macos/Info.plist'), /<key>CFBundleVersion<\/key>\s*<string>5<\/string>/)
...
assert.match(workspaceHTML, /class="writing-import-file-action">[\s\S]*Choose `\.md`…[\s\S]*id="writing-import-file" type="file" accept="\.md,text\/markdown,text\/plain"/)
...
assert.match(contents.get('packages/workspace/app/workspace-writing-import-ui.js'), /new TextDecoder\('utf-8', \{ fatal: true \}\)\.decode\(bytes\)/)
assert.doesNotMatch(contents.get('packages/workspace/app/workspace-writing-import-ui.js'), /fetch\(|XMLHttpRequest|FormData/)
...
assert.match(workspaceVisual, /'designOption\.createFromPlan'/)
...
assert.match(workspaceVisual, /executeStructural\(\s*'element\.gradient\.update'/)
