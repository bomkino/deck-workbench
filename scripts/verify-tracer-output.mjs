...
assert.deepEqual(records.map((record) => record.operation), ['command', 'undo', 'redo', 'undo', 'command', 'command'])
...
assert.equal(createResult.nativeSavePanel, true)
assert.equal(createResult.nativeFailurePresented, true)
assert.equal(createResult.journalReplayRevision, 3)
...
assert.equal(reopenResult.assemblyRevision, 6)
assert.equal(reopenResult.assemblyStyle, 'text-only')
assert.equal(reopenResult.assemblyReady, true)
assert.equal(reopenResult.corruptJournalFailure, 'JournalCorruption')
assert.equal(reopenResult.unsupportedSchemaFailure, 'UnsupportedSchema')
